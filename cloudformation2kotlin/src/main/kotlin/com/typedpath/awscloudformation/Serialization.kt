...
private fun mapProperties2GenericLocation(destination: CloudFormationTemplate, source: Any, namePrefix: String="") {
    fun prefixize(name: String) = if (!namePrefix.equals("")) (namePrefix + name.capitalize()) else name
    source.javaClass.kotlin.memberProperties.map { it -> Pair(it, it.get(source)) }
        .filter { it.second != null }.forEach {
            if (it.second is Resource) destination.resource(
                prefixize(it.first.name),
                it.second as Resource
            )
            else if (it.second is CloudFormationTemplate.Parameter) destination.parameter(
                prefixize(it.first.name),
                it.second as CloudFormationTemplate.Parameter
            )
            else if (it.second is CloudFormationTemplate.Output) destination.output(
                prefixize(it.first.name),
                it.second as CloudFormationTemplate.Output
            ) else if (it.second is CloudFormationTemplate.ResourceGroup) {
                mapProperties2GenericLocation(destination, it.second as Any, it.first.name)
            }
        }
}
...
fun toYaml(template: CloudFormationTemplate, newLine: String = "\r\n"): String {
    ...
    if (template.javaClass != CloudFormationTemplate::class.java) {
        mapProperties2GenericLocation(template, template)
    }
